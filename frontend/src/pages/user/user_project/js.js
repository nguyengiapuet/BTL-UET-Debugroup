...
function Pen() {
    ...
    const handleSavePens = async () => {
        try {
            if (params.id === undefined) {
                ...
                const response = await axios.put(`${SummaryApi.updatePens.url}/${params.id}`, {
                    title: dataPen.title,
                    html: dataPen.html,
                    css: dataPen.css,
                    js: dataPen.js,
    ...
    const handleLoadPens = async () => {
        try {
            const response = await axios.get(`${SummaryApi.getPens.url}/${params.id}`);
            if (response.data.success) {
    ...
    // Add Ctrl+S (or Cmd+S on Mac) to save and update the output
    useEffect(() => {
        const handleKeyDown = (e) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                e.preventDefault(); // Prevent default browser save dialog
                handleSavePens(); // Save the pen
                editCode(); // Update the output
            }
        };
    ...
    }, [dataPen]); // Dependency on dataPen to ensure latest state
        ...
        <div className="w-[calc(100wh-20px)] max-h-[calc(100vh-64px)] min-h-[calc(100vh-64px)] flex flex-col">
            <div className="h-[64px] bg-[#1D1C1C] flex items-center justify-between">
                <div className="h-[64px] flex items-center">
                    ...
                    <div className="">
                        ...
                        <h3 className="text-xs text-slate-400 font-normal">title</h3>
                    </div>
                </div>
            ...
            <div>
                <SplitPane split="horizontal" minSize={100} maxSize={-100} defaultSize={'50%'}>
                    <SplitPane split="vertical" minSize={100} maxSize={-100} defaultSize={'30%'}>
                        {/* Html */}
                            ...
                            <div className="h-full w-full relative p-1 bg-[#e5e7eb] pt-0 pr-0 pb-2">
                                <div className="flex justify-between bg-white absolute w-full">
                                    <div className="flex items-center px-2 bg-slate-300 gap-1">
                                        ...
                                        <p className="text-lg font-medium ">JS</p>
                                    </div>
                                    <div className="flex gap-2 items-center px-2">
                                        <FaCog className="text-base" />
                                        <FaChevronDown className="text-base" />
                    ...
                    <div className="w-full h-full bg-black border-2 border-black">
                        <iframe srcDoc={dataPen.output} className="text-white w-full min-h-full" />
                    </div>
                </SplitPane>
            </div>
        </div>
