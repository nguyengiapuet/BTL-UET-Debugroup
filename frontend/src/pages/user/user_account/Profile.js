...
function Profile() {
    ...
    const handleUpdateProfile = async () => {
        try {
            const response = await axios.put(`${SummaryApi.updateProfile.url}/${userData.id}`, {
            ...
            if (response.data.success) {
                ...
                setUserData({ ...userData, username: data.username, avatar: data.avatar });
            }
        } catch (error) {
        ...
        <div className=" w-full h-[90vh] bg-slate-100 flex justify-center items-center flex-col gap-4">
            <div className="h-fit w-[75%] bg-slate-200 rounded-xl px-4 py-4">
                <div className="flex items-center gap-36">
                    <div>
                        <div className="text-3xl text-[#9C6317] font-medium">Your profile info</div>
                        <p className="w-[600px] text-lg text-[#505050]">
                            Personal info and options to manage it. You can make some of this basic info, like your
                            contact details, visible to others so they can reach you easily.
                        </p>
                    </div>
                ...
                <div className="mt-12 flex flex-col gap-4">
                    <div className="flex gap-8 items-center">
                        <label className="text-[#505050] text-lg w-32">Profile picture</label>
                        <div className="text-[#505050] text-lg">Add a profile picture to your account</div>
                    ...
                    <div className="flex gap-8 items-center w-fit relative">
                        <label className="text-[#505050] text-lg w-32">Name:</label>
                        <input
                            type="text"
                            name="username"
                            value={data.username}
                    ...
                    <div className="flex gap-8 items-center">
                        <label className="text-[#505050] text-lg w-32">Gender</label>
                        <div>
                            <input className="" name="gender" value={'male'} type="radio" />
                            <label className="ml-2 text-lg text-[#505050]">Male</label>
                        </div>
                        <div>
                            <input className="" name="gender" value={'female'} type="radio" />
                            <label className="ml-2 text-lg text-[#505050]">Female</label>
                        </div>
                    </div>
                </div>
                <button
            ...
            <div className="bg-slate-200 w-[75%] h-36 rounded-xl pl-4">
                <div className="flex justify-between gap-10">
                    <div>
                        <div className="text-3xl mt-2 text-[#9C6317] font-medium">Danger</div>
                        <p className="w-[600px] text-lg text-[#505050]">
                            Deleting your account will wipe all infomation about you and content you have created
                        </p>
                    </div>
                    ...
                    <div className="bg-[#979797] w-full h-36 rounded-tr-xl rounded-br-xl flex items-center justify-center">
                        <div className="bg-[#9C6317] flex items-center gap-2 p-2 rounded-md cursor-pointer hover:bg-opacity-85">
                            ...
                            <button className="text-white font-semibold text-2xl">DELETE</button>
                        </div>
                    </div>
                </div>
            </div>
