            ...
            {isNavOpen &&
                <div className="overflow-hidden coding-box bg-[#ffffff] w-[300px] min-h-screen max-h-screen flex flex-col items-center gap-7 px-6 py-5">
                    ...
                    <div className="relative bg-[#EDEDED] rounded-full w-full flex py-[5px] items-center justify-between">
                        {/* Slider background */}
                        <div
                            className={`absolute top-[4px] bottom-[4px] left-[3px] right-[3px] w-[calc(50%-6px)] bg-white rounded-full shadow-xl transition-transform duration-300 ${activeButton === 'Following' ? 'translate-x-full' : 'translate-x-[3px]'
                                }`}
                        ></div>

                        {/* Category Button */}
                        <button
                            className={`relative z-10 px-4 py-2 left-[3px] flex items-center justify-start gap-1 text-sm font-bold rounded-full ${activeButton === 'Category' ? 'text-gray-700' : 'text-gray-500'
                                }`}
                        ...
                        {/* Following Button */}
                        <button
                            className={`relative z-10 px-4 py-2 right-[3px] flex items-center justify-center gap-1 text-sm font-bold rounded-full ${activeButton === 'Following' ? 'text-gray-700' : 'text-gray-500'
                                }`}
                    ...
                    {/* Search button */}
                    <div
                        className={`w-full border-2 rounded-3xl flex items-center px-4 transition-colors duration-300 ${isFocused ? 'border-[#D68E2F] boder-1' : 'border-[#D9D9D9]'
                            }`}
                    >
